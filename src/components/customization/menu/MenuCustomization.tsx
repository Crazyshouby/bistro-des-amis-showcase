...
export const MenuCustomization = () => {
  ...
  const [menuHeaderImage, setMenuHeaderImage] = useState(images.menuHeaderImage || "");
  ...
  useEffect(() => {
    ...
    setMenuHeaderImage(images.menuHeaderImage || "");
    setTempImageUrl(images.menuHeaderImage || "");
  }, [images]);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const fileName = `menu_header_${Math.random().toString(36).substring(2, 11)}.${fileExt}`;
  ...
  const handleApplyChanges = async () => {
    try {
      ...
      await updateTheme({ 
        images: { menuHeaderImage: tempImageUrl } 
      });
      
      // Mise à jour de l'état local après sauvegarde
      setMenuHeaderImage(tempImageUrl);
      setChanged(false);
          ...
          <TabsContent value="content" className="pt-4">
            <div className="space-y-4">
              <p className="text-muted-foreground">
                Options de personnalisation pour le contenu du menu à venir...
              </p>
            </div>
          </TabsContent>
        </Tabs>
